...
import { AlwaysAndNeverDrawnElementInfo } from "../../../../components/trees/models-tree/internal/AlwaysAndNeverDrawnElementInfo";
...
import type { ElementInfo } from "../../../../components/trees/models-tree/internal/ModelsTreeQueryHandler";
...
describe("AlwaysAndNeverDrawnElementInfo", () => {
  ...
  it("immediately queries both always and never drawn element info if sets are not empty", async () => {
    ...
    using(new AlwaysAndNeverDrawnElementInfo(vp, queryProvider), (_) => {
    ...
    it(`updates cache when ${setType} drawn set is changed`, async () => {
      ...
      await using(new AlwaysAndNeverDrawnElementInfo(viewport, queryHandler, debounceTime), async (info) => {
        for (let i = 0; i < 2; ++i) {
          ...
          expect(queryHandler.queryElementInfo).to.have.callCount(i + 2);
