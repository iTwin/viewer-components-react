...
Vitest does not provide a practical way to launch and debug a specific test via `launch.json`. Instead, use the [Vitest extension](https://marketplace.visualstudio.com/items?itemName=vitest.explorer) (`vitest.explorer`) for VS Code. It integrates with the Test Explorer panel, allowing you to run and debug individual tests directly from the editor with breakpoints.
