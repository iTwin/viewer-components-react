...
describe("AlwaysAndNeverDrawnElementInfo", () => {
  ...
  function runTests(setType: "always" | "never") {
    ...
    it(`retrieves and caches ${setType} drawn elements by category`, async () => {
      ...
      await sinon.clock.tickAsync(SET_CHANGE_DEBOUNCE_TIME);
      for (let i = 0; i < 3; ++i) {
        const obs = alwaysAndNeverDrawnElementInfo.getElements({ setType, categoryId, modelId });
        await expect(firstValueFrom(obs)).to.eventually.deep.eq(new Set(elements));
        expect(queryHandler).to.be.calledOnce;
      }
    });
    ...
    it(`updates cache when ${setType} drawn set is changed`, async () => {
      ...
      for (let i = 0; i < 2; ++i) {
        ...
        await expect(firstValueFrom(alwaysAndNeverDrawnElementInfo.getElements({ setType, modelId, categoryId }))).to.eventually.deep.eq(newSet);
      }
    });
    ...
    it(`doesn't query elements if ${setType} drawn set gets empty`, async () => {
      ...
      const obs = alwaysAndNeverDrawnElementInfo.getElements({ setType, modelId, categoryId });
    ...
    it(`debounces frequent changes to the ${setType} drawn set`, async () => {
      ...
      expect(queryHandler).to.be.calledOnce;
      await expect(firstValueFrom(alwaysAndNeverDrawnElementInfo.getElements({ setType, modelId, categoryId }))).to.eventually.deep.eq(finalSet);
    });
    ...
    it("waits until debounce period is passed and resolves with the newest result", async () => {
      ...
      const resultPromise = firstValueFrom(alwaysAndNeverDrawnElementInfo.getElements({ setType, modelId, categoryId }));
      updateSet(firstSet);
      updateSet(secondSet);
      await sinon.clock.tickAsync(SET_CHANGE_DEBOUNCE_TIME);
