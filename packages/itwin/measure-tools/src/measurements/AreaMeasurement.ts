...
import type { BeButtonEvent, DecorateContext, GraphicBuilder, RenderGraphicOwner } from "@itwin/core-frontend";
...
export class AreaMeasurementSerializer extends MeasurementSerializer {
  ...
  public get measurementName(): string { return AreaMeasurementSerializer.areaMeasurementName; }
...
export class AreaMeasurement extends Measurement {
  ...
  public addPointToDynamicPolygon(point: Point3d): boolean {
    ...
    if (this.polygonPoints.length > 0 && this.polygonPoints[this.polygonPoints.length - 1].isAlmostEqual(point))
      return false;
    ...
    if (this.polygonPoints.length > 2 && this.polygonPoints[0].isAlmostEqual(this.polygonPoints[this.polygonPoints.length - 1])) {
  ...
  public closeDynamicPolygon(): boolean {
    if (!this.isDynamic || !this.isValidPolygon)
      return false;

    const polyPoints = this.polygonPoints;
    const isClosedPath = polyPoints[0].isAlmostEqual(polyPoints[polyPoints.length - 1]);
    if (!isClosedPath)
  ...
  public override getDecorationGeometry(_pickContext: MeasurementPickContext): GeometryStreamProps | undefined {
    ...
    if (this.isDynamic) {
      if (this.polygonPoints.length >= 3)
        return [IModelJson.Writer.toIModelJson(PointString3d.create(this.polygonPoints[0]))];
  ...
  private getSnapId(): string | undefined {
    ...
    if (this.isDynamic && this.polygonPoints.length < 3)
      return undefined;
  ...
  private addDynamicSnapGraphic(styleSet: StyleSet, context: DecorateContext): void {
    if (!this.isDynamic || this.polygonPoints.length === 0)
      return;

    // Add a graphic just for snapping to the first point if we can
    const firstPtSnapId = this.getSnapId();
    if (firstPtSnapId === undefined)
      return;
  ...
  public override decorate(context: DecorateContext): void {
    ...
    if (this.isDynamic) {
      ...
      if (this._dynamicEdge)
        this._dynamicEdge.decorate(context);
  ...
  private drawDynamicArea(styleSet: StyleSet, graphicBuilder: GraphicBuilder): void {
    if (!this._dynamicEdge || this.polygonPoints.length === 0 || !this.polygon.drawFillArea)
      return;
  ...
  protected override async getDataForMeasurementWidgetInternal(): Promise<MeasurementWidgetData> {
    ...
    const fPerimeter = IModelApp.quantityFormatter.formatQuantity(this._polygon.perimeter, lengthSpec);
    const fArea = IModelApp.quantityFormatter.formatQuantity(this._polygon.area, areaSpec);
  ...
  public override equals(other: Measurement, opts?: MeasurementEqualityOptions): boolean {
    ...
    if (otherArea === undefined || this.polygonPoints.length !== otherArea.polygonPoints.length)
      return false;
