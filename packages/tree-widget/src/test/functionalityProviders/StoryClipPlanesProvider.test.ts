...
import { StoryClipPlanesProvider, ToggledTopFitViewFunctionalityProvider } from "../../components/trees/FunctionalityProviders";
...
import { DataLink } from "../../components/trees/visibility/DataLink";
...
describe("StoryClipPlanesProvider", () => {
  ...
  it("should perform action for StoryClipPlanesProvider", async () => {
    ...
    // const isolateRoomsForStoriesStub = sinon.stub(SectioningUtil, "isolateRoomsForStories");
  ...
  it("should not create clip or perform toggle in StoryClipPlanesProvider when story has no element", async () => {
    ...
    assert.strictEqual(MessageManager.messages[0].briefMessage, TreeWidget.translate("clipSection.briefTimeoutMessage"));
    queryRoomsStub.restore();
    queryStoryRangeStub.restore();
