...
export class BboxDimensionsDecorator implements Decorator {
  ...
  public async setContext(instanceId: string): Promise<boolean> {
    ...
    const query = `SELECT bBoxHigh,bBoxLow,yaw,pitch,roll,origin FROM BisCore.SpatialElement WHERE ECInstanceId = ${instanceId}`;
    const result = await IModelApp.viewManager.selectedView?.iModel.queryRows(
      query,
    );
    ...
    // validate result
    const elem = result.rows[0] as SpatialElementQueryResult;
