...
import { SpatialContainmentTree } from "../components/trees/SpatialContainmentTree";
  ...
  it("SpatialContainmentTree renders correctly with children", () => {
    const wrapper = (
      <Provider store={TestUtils.store}>
        <SpatialContainmentTree iModel={connection.object} displayByType={false} setDisplayByType={(_displayByType: boolean) => { }} displayGuids={true} setIsDisplayGuids={(_display: boolean) => { }} enableVisibility={false} />
      </Provider>
    );
    mount(wrapper).should.matchSnapshot();
  });

  it("SpatialContainmentTree renders correctly with visibility", () => {
    const wrapper = (
      <Provider store={TestUtils.store}>
        <SpatialContainmentTree iModel={connection.object} displayByType={false} setDisplayByType={(_displayByType: boolean) => { }} displayGuids={true} setIsDisplayGuids={(_display: boolean) => { }} enableVisibility={true} />
      </Provider>
    );
    mount(wrapper).should.matchSnapshot();
  });
