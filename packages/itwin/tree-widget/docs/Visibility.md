...
- **`parentElementsPath` / hierarchy path** — the chain of parent element/category segments leading from a model down to a node. Always/never-drawn queries scoped by this path ("path-scoped") only see elements in that hierarchy branch, so element elsewhere in the same model/category does not affect the node's status.
