...
**Current behavior** (`getElementsVisibilityStatus` + `getCategoryVisibilityFromAlwaysAndNeverDrawnElements`):
...
6. If has children: query `AlwaysAndNeverDrawnElementInfoCache` for child elements in the opposite set → combine with element's own count → compute status via `getCategoryVisibilityFromAlwaysAndNeverDrawnElementsImpl({ totalCount, numberOfElementsInOppositeSet })`
...
7. **Per-category visibility computation:** For each category in descendant counts:
   ...
   b. Compute per-category status via `getCategoryVisibilityFromAlwaysAndNeverDrawnElementsImpl({ totalCount: categoryCount, numberOfElementsInOppositeSet })`
...
**Current behavior** (`getCategoryVisibilityFromAlwaysAndNeverDrawnElements` with `categoryId` branch):
...
3. Compute status via `getCategoryVisibilityFromAlwaysAndNeverDrawnElementsImpl({ totalCount, numberOfElementsInOppositeSet })`
