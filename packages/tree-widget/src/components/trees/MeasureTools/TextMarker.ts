...
export class TextMarker implements CanvasDecoration {
  ...
  public applyStyle(style: TextStyleProps) {
    ...
    if (undefined !== style.textFont) this.textFont = style.textFont;
    if (undefined !== style.textLineHeight) this.textLineHeight = style.textLineHeight;
    if (undefined !== style.textColor) this.textColor = ColorDef.fromTbgr(style.textColor);
    if (undefined !== style.boxPadding) this.boxPadding = style.boxPadding;
    if (undefined !== style.boxCornerRadius) this.boxCornerRadius = style.boxCornerRadius;
    if (undefined !== style.boxBorderWidth) this.boxBorderWidth = style.boxBorderWidth;
    if (undefined !== style.boxBorderColor) this._boxBorderColor = ColorDef.fromTbgr(style.boxBorderColor).toHexString();
    if (undefined !== style.boxColor) this.boxColor = ColorDef.fromTbgr(style.boxColor);
    if (undefined !== style.icon) this.createIcon(style.icon);
  }
