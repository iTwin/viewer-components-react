...
import { isPresentationTreeNodeItem, UnifiedSelectionTreeEventHandler, UnifiedSelectionTreeEventHandlerParams } from "@itwin/presentation-components";
...
export class VisibilityTreeEventHandler extends UnifiedSelectionTreeEventHandler {
  ...
  private filterSelectionItems(items: TreeNodeItem[]) {
    ...
    return items.filter((item) => isPresentationTreeNodeItem(item) && this._selectionPredicate!(item.key, item));
  }
