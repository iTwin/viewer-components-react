...
function gatherLayers(vp, isOverlay) {
    ...
    return layers.map((layer, idx) => ({
        ...
        isOverlay,
        layerIndex: idx,
        subLayers: layer.subLayers?.map((s) => (typeof s.toJSON === "function" ? s.toJSON() : s)) ?? undefined,
    }));
}
 ...
 */
export async function openMapLayersWidget() {
 ...
 */
export function toggleBackgroundMap(vp, enabled) {
 ...
 */
export async function setBaseMapType(vp, type, colorTbgr) {
    const coreCommon = await import("@itwin/core-common");
 ...
 */
export function setMapTransparency(vp, transparency) {
    const viewport = getViewport(vp);
 ...
 */
export function toggleTerrain(vp, enabled) {
    const viewport = getViewport(vp);
 ...
 */
export function getMapLayerInfo(vp) {
    const viewport = getViewport(vp);
 ...
 */
export async function attachMapLayer(vp, url, name, formatId, isOverlay, userName, password) {
    const { MapLayerSource } = await import("@itwin/core-frontend");
    const viewport = getViewport(vp);
 ...
 */
export function detachMapLayer(vp, name, isOverlay) {
    const viewport = getViewport(vp);
    const detached = [];
    const tryDetach = (overlay) => {
 ...
 */
export function setMapLayerVisibility(vp, name, visible, isOverlay) {
    const viewport = getViewport(vp);
    let updated = 0;
    const trySetVisibility = (overlay) => {
        const layers = overlay
            ? viewport.displayStyle.settings.mapImagery.overlayLayers
            : viewport.displayStyle.settings.mapImagery.backgroundLayers;
        for (let i = 0; i < layers.length; i++) {
            if (layers[i].name === name) {
                viewport.displayStyle.changeMapLayerProps({ visible }, { index: i, isOverlay: overlay });
                updated++;
            }
        }
    };
