...
import { getOptimalBatchSize } from "../../../../shared/internal/Utils.js";
...
export class CategoriesTreeVisibilityHelper extends BaseVisibilityHelper {
  ...
  public getDefinitionContainersVisibilityStatus(props: { definitionContainerIds: Id64Arg }): Observable<VisibilityStatus> {
    return this.getGroupedContainedCategories(props.definitionContainerIds).pipe(
      mergeMap(({ emptyCategories, topMostElementCategories }) => {
        return merge(
          from(emptyCategories).pipe(map((id) => (this.#viewport.viewsCategory(id) ? createVisibilityStatus("visible") : createVisibilityStatus("hidden")))),
    ...
    return from(modelElementsMap).pipe(
      mergeMap(([modelId, { elementIds, childrenWhichAreParents }]) =>
        this.getElementsVisibilityStatus({
          ...
          categoryId,
          parentElementsPath,
          computeOnlyOwnStatus: this.#idsCache.canHaveHiddenChildren()
            ? undefined
    ...
    return this.getGroupedContainedCategories(props.definitionContainerIds).pipe(
      mergeMap(({ emptyCategories, topMostElementCategories }) => {
        if (emptyCategories.length > 0) {
          this.#viewport.changeCategoryDisplay({ categoryIds: emptyCategories, display: props.on });
        }
  ...
  public changeSubCategoriesVisibilityStatus(props: { categoryId: Id64String; subCategoryIds: Id64Arg; on: boolean }): Observable<void> {
    return concat(
      ...
      from(props.subCategoryIds).pipe(map((subCategoryId) => this.#viewport.changeSubCategoryDisplay({ subCategoryId, display: props.on }))),
    );
  }
  ...
  private getGroupedContainedCategories(
    ...
    return this.#idsCache.getAllContainedCategories({ definitionContainerIds }).pipe(
      reduce(
  ...
  private enableCategoryWithoutEnablingOtherCategories({ categoryId }: { categoryId: Id64String }): Observable<void> {
    this.#viewport.changeCategoryDisplay({ categoryIds: categoryId, display: true });
    return this.#idsCache.getModels({ categoryId }).pipe(
      mergeMap((modelId) => {
        this.#viewport.setPerModelCategoryOverride({ modelIds: modelId, categoryIds: categoryId, override: "none" });
        return this.#viewport.viewsModel(modelId)
          ? EMPTY
          : this.#idsCache.getCategories({ modelId }).pipe(
              map((allModelCategories) => {
                ...
                for (const modelCategoryId of allModelCategories) {
                  if (modelCategoryId !== categoryId) {
                    this.#viewport.setPerModelCategoryOverride({ modelIds: modelId, categoryIds: modelCategoryId, override: "hide" });
                  }
                }
                return modelId;
              }),
      ...
      map((hiddenModels) => {
        if (hiddenModels.length > 0) {
          this.#viewport.changeModelDisplay({ modelIds: hiddenModels, display: true });
        }
      }),
    );
  }
