...
describe("VisibilityTreeRenderer", () => {
  ...
  describe("createVisibilityTreeNodeRenderer", () => {
    it("renders nodes with default values", async () => {
      const { getByTestId, rerender } = render(createVisibilityTreeNodeRenderer({ iconsEnabled: false, descriptionEnabled: false })({ node: rootNode, treeActions }));
    ...
    it("disables expander for root node when `disableRootNodeCollapse` is set to true", async () => {
      const { getByTestId, rerender } = render(createVisibilityTreeNodeRenderer({ iconsEnabled: false, descriptionEnabled: false, disableRootNodeCollapse: true })({ node: rootNode, treeActions }));
      ...
      rerender((createVisibilityTreeNodeRenderer({ iconsEnabled: false, descriptionEnabled: false, disableRootNodeCollapse: true })({ node: middleNode, treeActions })));
      ...
      rerender((createVisibilityTreeNodeRenderer({ iconsEnabled: false, descriptionEnabled: false, disableRootNodeCollapse: true })({ node: leafNode, treeActions })));
    ...
    it("renders nodes with custom `levelOffset` value", async () => {
      const { getByTestId, rerender } = render(createVisibilityTreeNodeRenderer({ iconsEnabled: false, descriptionEnabled: false, levelOffset: 10 })({ node: rootNode, treeActions }));
      const renderedRootNode = await waitFor(() => getByTestId("tree-node-contents"));
      expect((renderedRootNode.children[1] as HTMLDivElement).style.marginRight).to.be.eq("0px");
