...
describe("ModelsTree", () => {
  ...
  describe("#unit", () => {
    ...
    const setupDataProvider = (nodes: PresentationTreeNodeItem[]) => {
    ...
    describe("<ModelsTree />", () => {
      ...
      it("should match snapshot", async () => {
        setupDataProvider([{
          id: "test",
          key: { type: "test-node-key", pathFromRoot: [], version: 0 },
          label: PropertyRecord.fromString("test-node"),
      ...
      it("calls visibility handler's `changeVisibility` on node checkbox state changes to 'checked'", async () => {
        ...
        setupDataProvider([node]);
        visibilityHandlerMock.setup((x) => x.getVisibilityStatus(moq.It.isAny())).returns(() => ({ state: "hidden" }));
        visibilityHandlerMock.setup(async (x) => x.changeVisibility(node, true)).returns(async () => { }).verifiable();
