...
import { ComponentIndex } from "../components/trees/ComponentIndex";
  ...
  it("ComponentIndex renders correctly with children", () => {
    const wrapper = (
      <Provider store={TestUtils.store}>
        <ComponentIndex iModel={connection.object} displayGuids={true} setIsDisplayGuids={(_display: boolean) => { }} enableVisibility={false} />
      </Provider>
    );
    mount(wrapper).should.matchSnapshot();
  });

  it("ComponentIndex renders correctly with visibility", () => {
    const wrapper = (
      <Provider store={TestUtils.store}>
        <ComponentIndex iModel={connection.object} displayGuids={true} setIsDisplayGuids={(_display: boolean) => { }} enableVisibility={true} />
      </Provider>
    );
    mount(wrapper).should.matchSnapshot();
  });
