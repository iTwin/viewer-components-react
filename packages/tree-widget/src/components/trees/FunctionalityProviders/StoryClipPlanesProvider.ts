  ...
  private async createSectionPlane(nodeInstanceId: string): Promise<boolean> {
    ...
    if (vp) {
      if (nodeInstanceId) {
        ...
        if (!sectionCreated) {
          const message: NotifyMessageDetailsType = new NotifyMessageDetails(OutputMessagePriority.Info, BuildingUIComponents.translate("clipSection.briefTimeoutMessage"), BuildingUIComponents.translate("clipSection.detailedTimeoutMessage"), OutputMessageType.Toast);
