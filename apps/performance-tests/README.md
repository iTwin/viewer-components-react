...
The simplest way to accommodate that is to use a custom vitest reporter (defined in `TestReporter.ts`).
The reporter gathers test durations and information about main thread blocking and saves it to a file if the `BENCHMARK_OUTPUT_PATH` environment variable is set.

Example: `BENCHMARK_OUTPUT_PATH="./results.json" vitest run`
