  ...
  public async zoomSelected(node: TreeModelNode) {
    ...
    if (NodeKey.isInstancesNodeKey(elementKey)) {
      ...
      if (data.length === 0) {
        ...
        if (child.length === 0) {
          const message: NotifyMessageDetailsType = new NotifyMessageDetails(OutputMessagePriority.Info, BreakdownTrees.translate("BreakdownTrees:zoomToElement.briefTimeoutMessage"), BreakdownTrees.translate("BreakdownTrees:zoomToElement.detailedTimeoutMessage"), OutputMessageType.Toast);
